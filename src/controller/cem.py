    ...
    def forward(
        ...
        for iter in range(self._iters):
            ...
            for t in range(self._horizon):
                ...
                # info update
                info["prev_state"] = self._state_seq_batch[:, prev_index, :]
                info["prev_action"] = samples[:, prev_index, :]
            ...
            if min_cost < best_cost:
                best_cost = min_cost
                best_actions = samples[min_idx].clone() # (horizon, dim_control)
            
            elite_idx = torch.topk(-total_cost, k=self._elite_num, sorted=False).indices
            elites = samples[elite_idx]  # (elite_num, horizon, dim_control)
